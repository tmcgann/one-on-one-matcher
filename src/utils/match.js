...
function makeMatches(allPersonsDict, personsAlreadyMatchedSet, personsToMatch) {
  const newMatches = []
  ...
  personsToMatch.forEach(person => {
    ...
    const allOtherPersonsNotAlreadyMatched = Object.keys(allPersonsDict).filter(
      personId =>
        !personsAlreadyMatchedSet.has(personId) &&
    ...
    const oldestMatchOfOldestMatch = getOldestUnmatchedMatch(
      personsAlreadyMatchedSet,
      oldestMatchPerson,
    )
    if (
